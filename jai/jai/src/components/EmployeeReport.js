...
const EmployeeReport = () => {
  ...
  const handleChange = (e) => {
    ...
    const [projectName, subCategoryName] = name.split("_");
      ...
      if (projectIndex > -1) {
        ...
        if (subCategoryName === "description") {
          ...
          if (subCategoryIndex > -1) {
            project.subCategory[subCategoryIndex].report = value;
          } else {
            project.subCategory.push({ subCategoryName, report: value });
          }
        }
        return [...prevState.slice(0, projectIndex), project, ...prevState.slice(projectIndex + 1)];
      } else {
        const newProject = {
          projectName,
